...
public class DimensionDoor extends JavaPlugin {
  ...
  public boolean onCommand(CommandSender sender, Command cmd, String commandLabel, String[] args) {
    if (cmd.getName().equalsIgnoreCase("dd")) {
      ...
      if (!commands.contains(command))
        return false;
      if (!playerHasPermission(sender, "dimensiondoor." + command))
        return true;
  ...
  private boolean createWorld(CommandSender sender, String[] args) {
    ...
    if (args.length < 3) {
      sender.sendMessage(ChatColor.RED + "/dd create [world] [type] <seed>");
      return true;
    }
  ...
  private boolean loadWorld(CommandSender sender, String[] args) {
    // check if we have enough arguments
    if (args.length != 2) {
      sender.sendMessage(ChatColor.RED + "/dd load [world]");
      return true;
    }
    // check to see if destination world is loaded
    if (WorldRecord.isLoaded(args[1])) {
  ...
  private boolean modifyWorld(CommandSender sender, String[] args) {
    // check if we have enough arguments
    if (args.length != 4) {
      sender.sendMessage(ChatColor.RED + "/dd modify [world] [attribute] [value]");
      return true;
    }
    // check to see if destination world is loaded
    if (!WorldRecord.isManaged(args[1])) {
  ...
  private boolean removeWorld(CommandSender sender, String[] args) {
    // check if we have enough arguments
    if (args.length != 2) {
      sender.sendMessage(ChatColor.RED + "/dd remove [world]");
      return true;
    }
    // check to see if destination world is loaded
    if (!WorldRecord.isManaged(args[1])) {
  ...
  private boolean teleportToWorld(CommandSender sender, String[] args) {
    // check if we have enough arguments
    if (args.length != 2) {
      sender.sendMessage(ChatColor.RED + "/dd teleport [world]");
      return true;
    }
    // check to see if destination world is loaded
    if (!WorldRecord.isLoaded(args[1])) {
    ...
    if (getName(sender).equals("console")) {
  ...
  private boolean unloadWorld(CommandSender sender, String[] args) {
    // check if we have enough arguments
    if (args.length != 2) {
      sender.sendMessage(ChatColor.RED + "/dd unload [world]");
      return true;
    }
    // check to see if destination world is loaded
    if (!WorldRecord.isLoaded(args[1])) {
