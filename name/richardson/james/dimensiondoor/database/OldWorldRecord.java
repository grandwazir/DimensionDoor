...
public class OldWorldRecord {
  ...
  static public int count(final String worldName) {
    return plugin.getDatabase().find(OldWorldRecord.class).where().ieq("name", worldName.toLowerCase()).findRowCount();
  }

  static public int count(final World world) {
    return plugin.getDatabase().find(OldWorldRecord.class).where().ieq("name", world.getName().toLowerCase()).findRowCount();
  }

  static public void create(final String worldName) throws WorldIsNotLoadedException {
    final OldWorldRecord managedWorld = new OldWorldRecord();
  ...
  static public List<OldWorldRecord> findAll() {
    final List<OldWorldRecord> worlds = plugin.getDatabase().find(OldWorldRecord.class).findList();
    return worlds;
  }

  static public OldWorldRecord findFirst(final String worldName) throws WorldIsNotManagedException {
    final List<OldWorldRecord> worlds = plugin.getDatabase().find(OldWorldRecord.class).where().ieq("name", worldName).findList();
    if (worlds.isEmpty())
      throw new WorldIsNotManagedException();
    return worlds.get(0);
  }

  static public OldWorldRecord findFirst(final World world) throws WorldIsNotManagedException {
    final List<OldWorldRecord> worlds = plugin.getDatabase().find(OldWorldRecord.class).where().ieq("name", world.getName()).findList();
    if (worlds.isEmpty())
      throw new WorldIsNotManagedException();
    return worlds.get(0);
  }
  ...
  public static void setup(final DimensionDoor plugin) {
    OldWorldRecord.plugin = DimensionDoor.getInstance();
  }
